...
fn parse(line: &str) -> Line {
    parse!(line, String, "position: " ; i32)
}
// ------------------------------------------------------- //
...
fn part1(mut player_1: Player, mut player_2: Player) {
    let mut die = Die::new();

    loop {
        if let Win = player_1.take_turn(&mut die) {
            println!("{}", player_2.score * die.roll_count);
            break;
        }

        if let Win = player_2.take_turn(&mut die) {
            println!("{}", player_1.score * die.roll_count);
            break;
        }
    }
}
